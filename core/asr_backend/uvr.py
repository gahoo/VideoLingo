...
from core.utils.models import _RAW_AUDIO_FILE, _VOCAL_AUDIO_FILE, _BACKGROUND_AUDIO_FILE, _AUDIO_DIR
...
def uvr_audio():
    if os.path.exists(_VOCAL_AUDIO_FILE) and os.path.exists(_BACKGROUND_AUDIO_FILE):
        rprint(f"[yellow]⚠️ {_VOCAL_AUDIO_FILE} and {_BACKGROUND_AUDIO_FILE} already exist, skip UVR processing.[/yellow]")
        return

    rprint("🤖 Initializing UVR audio separation...")
    
    # Load UVR configuration from config.yaml
    uvr_config = load_key("vocal_separation.uvr")
    api_url = uvr_config.get("url")
    model = uvr_config.get("model")
    
    if not api_url or not model:
        rprint("[red]❌ UVR API URL or model not configured in config.yaml under the 'vocal_separation.uvr' key. Skipping separation.[/red]")
        return

    # Setup client
    logger = logging.getLogger(__name__)
    # Assuming logger is configured by the application's entry point.
    
    api_client = AudioSeparatorAPIClient(api_url, logger)

    rprint(f"🎵 Separating audio with model: {model}...")
    
    os.makedirs(_AUDIO_DIR, exist_ok=True)

    # Call the API to separate audio
    result = api_client.separate_audio_and_wait(
        file_path=_RAW_AUDIO_FILE,
        model=model,
    ...
    if result.get("status") == "completed":
        rprint("[green]✨ Audio separation completed![/green]")
        downloaded_files = result.get("downloaded_files", [])
        
        vocal_file = None
        background_file = None
        
        # Find vocal and background files from the downloaded list
        remaining_files = []
        for file_path in downloaded_files:
            filename_lower = os.path.basename(file_path).lower()
            if 'vocal' in filename_lower and not vocal_file:
                vocal_file = file_path
            elif ('instrumental' in filename_lower or 'no_vocal' in filename_lower) and not background_file:
        ...
        # Rename vocal file
        if vocal_file:
            rprint(f"🎤 Found vocal file: {os.path.basename(vocal_file)}. Renaming to {os.path.basename(_VOCAL_AUDIO_FILE)}")
            shutil.move(vocal_file, _VOCAL_AUDIO_FILE)
        else:
            rprint("[red]❌ Vocal file not found in separation result. Aborting.[/red]")
            # Clean up all downloaded files before aborting
            for file_path in downloaded_files:
        ...
        if background_file:
            rprint(f"🎹 Found background file: {os.path.basename(background_file)}. Renaming to {os.path.basename(_BACKGROUND_AUDIO_FILE)}")
            shutil.move(background_file, _BACKGROUND_AUDIO_FILE)
        else:
            rprint("[yellow]⚠️ Background music file not found in separation result.[/yellow]")
        ...
        for file_path in remaining_files:
            if os.path.exists(file_path):
                try:
                    os.remove(file_path)
                    rprint(f"🧹 Cleaned up temporary file: {os.path.basename(file_path)}")
                except OSError as e:
                    rprint(f"[red]❌ Error cleaning up file {file_path}: {e}[/red]")

    else:
        rprint(f"❌ Separation failed: {result.get('error', 'Unknown error')}")